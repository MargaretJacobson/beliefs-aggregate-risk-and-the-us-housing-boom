...
recursive double precision FUNCTION GOLDEN(AX,BX,CX,F,TOL,XMIN)
...
!print*, 'In golden: AX, X1,X2,CX',X0,X1, X2,X3

F1=F(X1)
F2=F(X2)  !Initial function evaluations
!print*,'Initial function evaluations',F(X0), F1, F2, F(X3)
1 IF(ABS(X3-X0).GT.TOL*(ABS(X1)+ABS(X2)) .AND. ( (ABS(X1) + ABS(X2)) .GT. 1.0d-12)) THEN
	IF(F2.LT.F1) THEN
	  X0=X1
	  X1=X2
	  X2=R*X1+C*X3
